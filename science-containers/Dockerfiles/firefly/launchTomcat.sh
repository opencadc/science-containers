...
EXTENSIONS_DIR=${EXTENSIONS_DIR:-${CATALINA_HOME}/extensions}
...
echo "        Extensions directory             EXTENSIONS_DIR                ${EXTENSIONS_DIR}"
echo
...
echo -e "============================================================"
echo -e "         Firefly Runtime Configuration"
echo -e "============================================================"
echo "Starting WAR extraction process..."
for archive in ${CATALINA_HOME}/webapps-ref/*.war; do
    echo "  Discovered archive: ${archive}"
    warName=$(basename "${archive}" .war)
    # If baseURL is set, append it to the context path
    if [[ -n "${baseURL}" ]]; then
      warName="$(sed -r 's/\//#/g;s/^(#)+|(#)+$//g' <<< "${baseURL}")#${warName}"
    fi

    # Set the warDir to the webapps directory
    warDir="${CATALINA_HOME}/webapps/${warName}"

    # If warDir exists, inform user and skip, else extract web archive
    if [[ -d "${warDir}" ]]; then
      echo "  Skipping extraction, ${warDir} already exists"
    else
      echo "  Extracting ${archive} to ${warDir}"
      mkdir -p "${warDir}"
      unzip -oqd "${warDir}" "${archive}"
      sed -E -i.bak 's/##out--//' "${warDir}/WEB-INF/classes/log4j2.properties"
    fi
done
echo "  WAR extraction process complete."
echo "Configuring third party extensions for Firefly..."
# Copy .jar files from CATALINA_HOME/extensions to CATALINA_HOME/firefly/WEB-INF/lib/
if [[ -d ${EXTENSIONS_DIR} ]]; then
  # Check if firefly/WEB-INF/lib/ directory exists
  if [[ ! -d "${CATALINA_HOME}/webapps/firefly/WEB-INF/lib" ]]; then
    echo "  Creating ${CATALINA_HOME}/webapps/firefly/WEB-INF/lib/"
    mkdir -p "${CATALINA_HOME}/webapps/firefly/WEB-INF/lib"
  fi

  for extension in "${CATALINA_HOME}/extensions/"*.jar; do
    cp -v "${extension}" "${CATALINA_HOME}/webapps/firefly/WEB-INF/lib/"
  done

else
  echo "  ${EXTENSIONS_DIR} does not exist, nothing to configure."
fi
echo "  Third party extension configuration complete."
echo -e "============================================================\n"
